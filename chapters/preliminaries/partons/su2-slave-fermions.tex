...
The idea of \ref{Abrfer} is to identify $\mathcal{H}_{S=\frac12}$ with the one-particle sector of $\mathcal{H}_{\text{fermions}}$. This leaves two un-physical states: $\ket{00}$ and $\ket{11}$. The spin-operators $S^\alpha$ act trivially on these, which is why everything goes through smoothly on the operator level. \\


Therefore, as long as the Hamiltonian only consists of spin operators, the un-physical states do not influence any physical results (apart from normalization, maybe). This can not garuanteed anymore when treating the system perturbatively using for example a mean-field theory. For this reason we introduce the constraint 

\begin{equation}
\bra{\psi} n_{i\downarrow} + n_{i\uparrow}\ket{\psi} = 1,
\end{equation}

which is a constraint on the state $\ket{\psi}$. 
...
the spin rotation can be represented by $\vec{\sigma_2} \mapsto V^{-1}\vec{\sigma}V^{-1}$, with $V$ an element of SU(2). The task at hand is to determine, how the spin rotation acts on the ladder operators. One may reinterpret 
...
$c_\downarrow, \, c_\uparrow$ transform as a SU(2) doublet under left multiplication by an SU(2) matrix. To cosntruct the conjugate doublet, which transforms in the same manner, consider 
...
c_\downarrow^\dagger & 	-c_\uparrow 		 
\end{pmatrix}
\end{equation}
