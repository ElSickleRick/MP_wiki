...
the spin rotation can be represented by $\vec{\sigma} \mapsto V^{-1}\vec{\sigma}V$, with $V$ an element of SU(2). The task at hand is to determine, how the spin rotation acts on the ladder operators. One may reinterpret 
...
It is convention two write these two doublets in a matrix:
...
\end{equation}
	...
	the spin rotation is then $\psi \mapsto \psi V$. Hence, the combination $\psi_i\psi_j^\dagger$ is manifestly invariant. This reveals two rotationally invariant operators $\chi_{ij}$ and $\eta_{ij}$ : 

\begin{equation}
	\psi\psi^\dagger = 
	\begin{pmatrix}
		c_{i\uparrow}^\dagger c_{j\uparrow}^\dagger + c_{i\downarrow}c_{j\downarrow}^\dagger & c_{i\uparrow}c_{j\downarrow} - c_{i\downarrow}c_{j\uparrow} \\
		c_{i\downarrow}^\dagger c_{j\uparrow}^\dagger - c_{i\uparrow}^\dagger c_{j\downarrow}^\dagger & c_{i\downarrow}^\dagger c_{j\downarrow} + c_{i\uparrow}^\dagger c_{j\uparrow}
	\end{pmatrix} \equiv
	\begin{pmatrix}
		- \chi_{ij}^\dagger & - \eta_{ij}^\dagger \\
		- \eta_{ij} & \chi_{ij}
	\end{pmatrix}
\end{equation}

The spint operators can be expressed in manifestly rotationally-invariant form:

\begin{equation}
	S_i^\alpha = \frac12 \text{Tr}\left[\psi_i^\dagger \psi_i \left(\sigma\right)^T\right]
\end{equation}
...
\begin{equation}\label{gauge_invar}
...
On the level of local hilbert spaces, this can be understood as different walks connecting the same initial and final states (hence, an artifact of the enlargement).

\begin{center}
	\includegraphics[width = 0.35\textwidth]{raising_operation.jpeg}.
\end{center}

The physical interpretation is that $c_\downarrow$ and $-c_\uparrow^\dagger$ hace the same action: they decrease $S_z$ by unit. Suddenly, we have a gauge-structure at our hands. The next questions come naturally: what are general gauge transformations and what are gauge invariants? We conbine the four operators into two doublets:
...
which transform by left multiplication by a matrix (the gauge transformation). These doublets again form \ref{gauge_invar}, From the representation of the spin-operators it is obvious that the gauge transformatoins have to be from $SU(2)$.
