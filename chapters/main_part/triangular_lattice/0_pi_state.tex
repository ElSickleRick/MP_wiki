...
\begin{align}
	\begin{split}
		...
		&+\mu_a a_{i\sigma}^\dagger a_{i\sigma} + \mu_b b_{i\sigma}^\dagger b_{i\sigma} \bigg) - \frac{N(\mu_a + \mu_b)}{2}
	\end{split}
\end{align}
...
\subsubsection{small momentum expansion}
...
to obtain 
\begin{equation}
	\Rightarrow H_{\vec{K}}  \approx 2t \sum_{p, \sigma}
	\begin{pmatrix}
		a_{p\sigma}^\dagger & b_{p\sigma}^\dagger
	\end{pmatrix}
	\begin{pmatrix}
...
\begin{equation}
	\Rightarrow H_{\vec{K}^\prime}  \approx 2t \sum_{p, \sigma}
	\begin{pmatrix}
		a_{p\sigma}^\dagger & b_{p\sigma}^\dagger
	\end{pmatrix}
	\begin{pmatrix}
...
Thus, the fermi velocity is $v_F = \sqrt{6}t$. This result has been reported before here \cite{Rachel2015}. \textcolor{blue}{ $\vec{K}$ and $\vec{K}^\prime$ are time revearsel partners, thus the two hamiltonains should map onto each other under time revearesal. I would guess that $T = -C$ in k-space, where C is complex conjugation, which would be consistent with the form of the Hamiltonian.} \\ 

\subsubsection{Continuum Model/ Gradient Expansion}

Next I will derive the low-energy Hamilotnian in real space, as a continuum theory (/field theory). I start by writing down the real-space hamiltonian, I reshuffled term a bit compared to when I first wrote it down above, the chemical potentials are set to zero.
...
\begin{align}
	\begin{split}
		H = \frac{t}{2} \sum_{i \sigma} &\bigg( a_{i\sigma}^\dagger a_{i+\delta_1;\sigma}  - a_{i\sigma}^\dagger b_{i+\delta_2;\sigma} + a_{i\sigma}^\dagger b_{i+\delta_3;\sigma} - b_{i\sigma}^\dagger b_{i+\delta_1;\sigma} + b_{i\sigma}^\dagger a_{i+\delta_2;\sigma} + b_{i\sigma}^\dagger a_{i+\delta_3;\sigma} + \text{h.c.} \bigg)  \\
	\end{split}
\end{align}

\textcolor{red}{I chose a bad method for indexing the lattice position, I should eventually revise this.} \\
...
$A_{K\sigma}, A_{K^\prime\sigma}, B_{K\sigma}$ aqnd $ B_{K^\prime\sigma}$ are slowly varying fermionic fields. I begin with the expansion around the $K$-point. \textcolor{blue}{If the above expansion is correct, the cross terms should vanish. Show this.} The protoypical method is 
