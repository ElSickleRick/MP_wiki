...
(the minus comes from minimal coupling). It turns out however, that $\vec{B}=0$, which is explained in what follows. In general, however

\begin{equation}
	B = \left(\partial_x A_y - \partial_y A_x\right)
\end{equation}

The first term is simply rotationless (this was shown here \cite{Ramezani_Masir2013}) and can therefore be "gauged away". \\
...
Evidently each expression in $\vec{h}$ is obtained by adding contribtions from hopping along $\vec{\delta}_{ij}$ and along the opposite direction ($-\vec{\delta}_{ij}$) (this can be seen nicely by considering that in the directions of $\vec{\delta}_3$ and $\vec{\delta}_2$ one simply has tight-binding chains, leading to the well-known cosine dispersion). \\

For the strain expansion one would have to break these terms (e.g. the cosisne terms) up again into the contributions from hopping in opposite directions. However, since $\vec{\delta}_{ij} \cdot \underline{u} \cdot \vec{\delta}_{ij}$ is invariant under $\vec{\delta}_{ij} \mapsto -\vec{\delta}_{ij}$ it acquires the same dispersion-like prefactor as the zeroth order, which vanishes by definition at $\vec{K}^{(\prime)}$. \\

So why does the term vanish? Its a mix of the special structure of the hamiltonian (i.e. the local directions decouple in the pauli-matrix basis) and the fact that the expansion term $\vec{\delta}_{ij} \cdot \underline{u} \cdot \vec{\delta}_{ij}$ cant distinguish between $\vec{\delta}_{ij}$ and $-\vec{\delta}_{ij}$. \textcolor{blue}{The same should hold for the $\pi$-flux square lattice. Graphene shows none of these features (the symmetry of the term doesnt change of course but therer is simply no hopping in the opposite direction). For Kagome the first feature is absent.} \\
