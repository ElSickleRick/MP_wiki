...
\begin{align}
	\cos(\vec{\delta}_1(\vec{K} + \vec{p})) &\approx \cos(\vec{\delta}_1\vec{K}) - \sin(\vec{\delta}_1\vec{K})\vec{\delta}_1\vec{p} = - \vec{\delta}_1\vec{p}\\
	\sin(\vec{\delta}_2(\vec{K} + \vec{p})) &\approx \sin(\vec{\delta}_2\vec{K}) + \cos(\vec{\delta}_2\vec{K}) \vec{\delta}_2\vec{p} = \vec{\delta}_2\vec{p} \\
	\cos(\vec{\delta}_3(\vec{K} + \vec{p})) &\approx \cos(\vec{\delta}_3\vec{K}) - \sin(\vec{\delta}_3\vec{K}) \vec{\delta}_3\vec{p} = - \vec{\delta}_3\vec{p}
\end{align}
...
\begin{align}
	a_{i\sigma}^\dagger a_{i+\delta;\sigma} &\sim e^{i\vec{K}\vec{\delta}}A^\dagger_{K\sigma}(\vec{R}_i)A_{K\sigma}(\vec{R}_i + \vec{\delta}) \notag \\
...
\begin{gather}
	A^\dagger_{K\sigma}(\vec{R}_i)\left[e^{i\vec{K}\vec{\delta}_1}(1+i\vec{\delta}_1\vec{p}) + e^{-i\vec{K}\vec{\delta}_1}(1-i\vec{\delta}_1\vec{p})\right]A_{K\sigma}(\vec{R}_i) \label{mael11}\\
...
To introduce strain into the model, I first establish its effect on the coordinates and exchange couplings \textcolor{red}{Awkward formulation}
...
Furthermore it is convenient to redefine 
...
I do the same thing for the exchange couplings 
...
\begin{align}
	...
	e^{i\vec{\delta}_{ij}^\prime\vec{K}} &\approx e^{i\vec{\delta}_{ij}\vec{K}} + ie^{i\vec{\delta}_{ij}\vec{K}}\left( \vec{K} \cdot \underline{u} \cdot \vec{\delta}_{ij}
	\right)
\end{align}

\textcolor{red}{I think that the terms proportional to $\exp(i\vec{\delta}_{ij}) J(\vec{\delta}_{ij})$ (see matrix elements) vanish. But I still have to check that.}
The terms that "generate" the strain effects are of the form

\begin{equation}
	e^{i\vec{\delta}_{ij}^\prime\vec{K}}J(\vec{\delta}_{ij}^\prime)\vec{\delta}_{ij}^\prime, \quad i \in [1,2,3]
\end{equation}

\textcolor{red}{In this approximation the Mean field does not react to the strain at all??}

Accoring to the formulae above this generates the following 8 terms (arranged by order in $\underline{u}$):

\begin{align}
	&Je^{i\vec{\delta}_{ij}\vec{K}}\vec{\delta}_{ij} &  \\ \hline 
	&iJe^{i\vec{\delta}_{ij}\vec{K}}\left( \vec{K} \cdot \underline{u} \cdot \vec{\delta}_{ij} \right)  \vec{\delta}_{ij} & \text{contributes to A in Graphene}\\
	&-\frac{J\beta}{a^2}e^{i\vec{\delta}_{ij}\vec{K}}\left(\vec{\delta}_{ij} \cdot \underline{u} \cdot \vec{\delta}_{ij}\right) \vec{\delta}_{ij} & \text{contributes to A in Graphene} \\ 
	&Je^{i\vec{\delta}_{ij}\vec{K}}\left(\underline{u} \cdot \vec{\delta}_{ij}\right) &\text{\textcolor{red}{some sources say fermi velocity in Graphene?}} \\ \hline
	&-i\frac{J\beta}{a^2}e^{i\vec{\delta}_{ij}\vec{K}}\left( \vec{K} \cdot \underline{u} \cdot \vec{\delta}_{ij} \right) \left(\vec{\delta}_{ij} \cdot \underline{u} \cdot \vec{\delta}_{ij}\right) \vec{\delta}_{ij} & \text{contributes to A in Graphene} \\
	&-\frac{J\beta}{a^2}e^{i\vec{\delta}_{ij}\vec{K}}\left(\vec{\delta}_{ij} \cdot \underline{u} \cdot \vec{\delta}_{ij}\right) \left(\underline{u} \cdot \vec{\delta}_{ij}\right) \\
	& iJe^{i\vec{\delta}_{ij}\vec{K}} \left( \vec{K} \cdot \underline{u} \cdot \vec{\delta}_{ij} \right) \left( \underline{u} \cdot \vec{\delta}_{ij} \right) \\ \hline
	&-i\frac{J\beta}{a^2}e^{i\vec{\delta}_{ij}\vec{K}}\left( \vec{K} \cdot \underline{u} \cdot \vec{\delta}_{ij} \right) \left( \vec{\delta}_{ij} \cdot \underline{u} \cdot \vec{\delta}_{ij} \right)\left( \underline{u} \cdot \vec{\delta}_{ij} \right)
\end{align}

\textcolor{red}{This is not the full expansion up to third order! In total there are 6 terms in the second and 10 terms the third order.} I define 

\begin{equation}
	\xi_{\vec{K}}(\vec{\delta}_{ij}) \equiv \left(\underline{u} \cdot \vec{\delta}_{ij} \right)\cdot\left(i\vec{K} - \frac{J\beta}{a^2}\vec{\delta}_{ij}\right)
\end{equation}

\begin{equation}
	\Rightarrow J(\vec{\delta}_{ij})e^{i\vec{K}\vec{\delta}_{ij}}(1+i\vec{\delta}_{ij} \vec{p}) \approx (1+\xi_{\vec{K}}(\vec{\delta}_{ij}))J(\vec{\delta}_{ij})e^{i\vec{K}\vec{\delta}_{ij}}(1+i\vec{\delta}_{ij}\vec{p}) + J(\vec{\delta}_{ij})e^{i\vec{K}\vec{\delta}_{ij}} \left( \underline{u} \cdot \vec{\delta}_{ij} \right) 
\end{equation}
