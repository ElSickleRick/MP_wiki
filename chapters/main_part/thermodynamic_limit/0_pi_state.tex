...
\textcolor{red}{Nayga, Rachel and Vojta neglect prodcuts of order $u^{1}\cdot\vec{p}^{1}$. Why does that make sense? I dont know...}

The matrix elements of the strained Hamiltonian are of the form 

\begin{equation}
	\frac12\left[J(\vec{\delta}_{ij}^\prime)e^{i\vec{K}\vec{\delta}_{ij}^\prime}(1+i\vec{\delta}_{ij}^\prime\vec{p}) + J(-\vec{\delta}_{ij}^\prime)e^{-i\vec{K}\vec{\delta}_{ij}^\prime}(1-i\vec{\delta}_{ij}^\prime\vec{p})\right]
\end{equation}
...
\begin{equation}
	J(\vec{\delta}_{ij}^\prime)e^{i\vec{K}\vec{\delta}_{ij}^\prime}(1+i\vec{\delta}_{ij}^\prime\vec{p}) \approx Je^{i\vec{K}\vec{\delta}_{ij}}(1+i\vec{\delta}_{ij}\vec{p}) + iJe^{i\vec{\delta}_{ij}\vec{K}}\left(\vec{K} \cdot \underline{u} \cdot \vec{\delta}_{ij} \right) - \frac{J\beta}{a^2} e^{i\vec{\delta}_{ij}\vec{K}} \left( \vec{\delta}_{ij} \cdot \underline{u} \cdot \vec{\delta}_{ij} \right)
\end{equation}
...
\textcolor{red}{Maybe make progress by splitting the strain tensor into symmetric and antisymmetric parts and thereby obtain a term proportional to $\vec{\delta}_{ij}$ which would then be the effective vector potential.}
