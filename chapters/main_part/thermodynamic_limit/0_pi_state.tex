...
\begin{align}
	...
	&iJe^{i\vec{\delta}_{ij}\vec{K}}\left( \vec{K} \cdot \underline{u} \cdot \vec{\delta}_{ij} \right)  \vec{\delta}_{ij} & \text{contributes to A}\\
	&-\frac{J\beta}{a^2}e^{i\vec{\delta}_{ij}\vec{K}}\left(\vec{\delta}_{ij} \cdot \underline{u} \cdot \vec{\delta}_{ij}\right) \vec{\delta}_{ij} & \text{contributes to A} \\ 
	&Je^{i\vec{\delta}_{ij}\vec{K}}\left(\underline{u} \cdot \vec{\delta}_{ij}\right) &\text{\textcolor{red}{some sources say fermi velocity?}} \\ \hline
	&-i\frac{J\beta}{a^2}e^{i\vec{\delta}_{ij}\vec{K}}\left( \vec{K} \cdot \underline{u} \cdot \vec{\delta}_{ij} \right) \left(\vec{\delta}_{ij} \cdot \underline{u} \cdot \vec{\delta}_{ij}\right) \vec{\delta}_{ij} & \text{contributes to A} \\
...
\textcolor{red}{This is not the full expansion up to third order! In total there are 6 terms in the second and 10 terms the third order.}
