...
This is very similar to the $\pi-0-$flux-state. The bare Hamiltonian is
\begin{align}
	\begin{split}
		H  = \sum_{i,\sigma}\bigg(\frac{t}{2}\bigg[  &-ia_{i\sigma}^\dagger a_{i+\delta_1;\sigma} + ia_{i\sigma}^\dagger a_{i-\delta_1;\sigma} - a_{i\sigma}^\dagger b_{i+\delta_2; \sigma} + a_{i\sigma}^\dagger b_{i-\delta_2;\sigma}+ a_{i\sigma}^\dagger b_{i+\delta_3;\sigma} + a_{i\sigma}^\dagger b_{i-\delta_3;\sigma} \\
		&+ib_{i\sigma}^\dagger b_{i+\delta_1;\sigma} - i b_{i\sigma}^\dagger b_{i-\delta_1;\sigma} + b_{i\sigma}^\dagger a_{i+\delta_2;\sigma} - b_{i\sigma}^\dagger a_{i-\delta_2;\sigma} + a_{i\sigma}^\dagger b_{i+\delta_3;\sigma} + a_{i\sigma}^\dagger b_{i-\delta_3;\sigma}\bigg] \\
		&+\mu_a a_{i\sigma}^\dagger a_{i\sigma} + \mu_b b_{i\sigma}^\dagger b_{i\sigma}\bigg) - \frac{N(\mu_a + \mu_b)}{2} 
	\end{split}
\end{align}

\textcolor{red}{The site index notation is not ideal, as a and b sites by definition cant have the same site indices.}
...
\textcolor{red}{Maybe I should show explicitly that $\mu = 0$, I guess $\mu_a = - \mu_b = \mu$ is clear}
...
\begin{equation}
	\vec{h} = 2t
	\begin{pmatrix}
		\cos(\vec{q}\vec{\delta}_1) \\
		\sin(\vec{q}\vec{\delta}_2) \\
		\sin(\vec{q}\vec{\delta}_3)
	\end{pmatrix} \mapsto 2t
	\begin{pmatrix}
		\cos(k_y-k_x/2) \\
		\sin(k_x/2) \\
		\sin(k_y) 
...
I first simplify \ref{Chernform} a bit using

\begin{equation}
	\partial_\alpha \hat{h} = \frac{1}{\abs{\vec{h}}}\left(\partial_\alpha\vec{h} - \left(\hat{h}\cdot\partial_\alpha \vec{h}\right)\hat{h}\right).
\end{equation}
...
\begin{align}
	\Rightarrow \vec{h} \cdot \left( \partial_{q_x} \vec{h} \times \partial_{q_y} \vec{h} \right) &= 4 \sqrt{3} t^3\left( -\cos(\delta_1)\cos(\delta_2)\cos(\delta_3) + \cos(\delta_1)\sin(\delta_2)\sin(\delta_3) - \sin(\delta_1)\cos(\delta_2)\sin(\delta_3)\right) \notag \\
	&= 4 \sqrt{3} t^3\left(\sin(\delta_1)\sin(\delta_2)\cos(\delta_3) -1\right)
\end{align}

\begin{equation}
	\Rightarrow C = \frac{\sqrt{3}}{8\pi} \int\limits_{\text{1.BZ}} d^2q \frac{\sin(\delta_1)\sin(\delta_2)\cos(\delta_3)-1}{\left(\sin^2(\delta_1) + \sin^2(\delta_2) + \cos^2(\delta_3)\right)^{3/2}}  
\end{equation}

For evaluation I use the transformation:
...
\begin{equation}
	\Rightarrow C = \frac{1}{8\pi} \int\limits_{[-\pi,\pi]^2} d^2k \frac{\sin(k_y)\sin(k_y-k_x/2)\cos(k_x/2)-1}{\left(\sin^2(k_y) + \sin^2(k_y-k_x/2) + \cos^2(k_x/2)\right)^{3/2}}  
\end{equation}
